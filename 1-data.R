...
# read traits
dune.traits = read.table(file = "./dune 2/data_clean/dune_traits_Z.txt", header = T)
...
dat = mutate(dune.all, y = log(freq + 1))
traits = names(dune.traits2)[-1]
nspp = n_distinct(dat$sp)
nsite = n_distinct(dat$site)
phy <- drop.tip(dune.phylo2, tip = dune.phylo2$tip.label[!dune.phylo2$tip.label %in% unique(dat$sp)])
Vphy <- vcv(phy)
Vphy <- Vphy[order(phy$tip.label),order(phy$tip.label)]
Vphy <- Vphy/max(Vphy)
Vphy <- Vphy/det(Vphy)^(1/nspp) # this is the C matrix 
dat$sp = as.factor(dat$sp)
dat$site = as.factor(dat$site)
re.site <- list(1, site = dat$site, covar = diag(nsite))
re.sp <- list(1, sp = dat$sp, covar = diag(nspp))
re.sp.phy <- list(1, sp = dat$sp, covar = Vphy) # random term of sp, accounting for phylo. relationships
re.nested.phy <- list(1, sp = dat$sp, covar = Vphy, site = dat$site) # underdispersion
maxit = 5000 # number of interations
reltol = 1e-12
